...
def numerical_imputer(numerical, state="transform"):
    ...
    if state == "fit":
        ...
        joblib.dump(imputer, "output/preprocess_data/estimator/numerical_imputer.pkl")
...
def one_hot_encoder(x_cat, state="fit"):
    df = x_cat.copy()
    index = x_cat.index
    col = x_cat.columns
    ...
    if state == "fit":
        ...
        encoder.fit(x_cat)
        joblib.dump(encoder, "output/preprocess_data/estimator/onehotencoder.pkl")
...
def normalization(x_all, state="fit"):
    ...
    if state == "fit":
        ...
        joblib.dump(normalizer, "output/preprocess_data/estimator/normalizer.pkl")
