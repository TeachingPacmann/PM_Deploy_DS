...
def read_data(params):
    x_train = joblib.load(params['x_train_path'])
    y_train = joblib.load(params['y_train_path'])
    x_valid = joblib.load(params['x_valid_path'])
    y_valid = joblib.load(params['y_valid_path'])

    return x_train, y_train, x_valid, y_valid


def model_lasso():
    param_dist = {'alpha': [0.25, 0.5, 1, 1.5]}
